...
\section{Class 15}
\textbf{Concavity} - a function is concave if its second derivative is negative, or, if 
\begin{equation}
...
\textbf{Jensen's inequality} - generalizing for any point, we have 
\begin{equation}
    f(\mathbb{E}_{p(t)t}) \geq \mathbb{E}_{p(t)}f(t)
\end{equation}

\textbf{Kullback-Leibler Divergence} - is a way to measure the difference between two probabilistic functions.
\begin{equation}
